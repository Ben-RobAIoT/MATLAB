...
figure(1), clf, imshow(IN, 'XData', linspace(0,1,resolution), ... 
    'YData', linspace(0,1,resolution)); axis on; set(gca, 'YDir', 'Normal'); 
xlabel('x'); 
ylabel('y'); 
...
Sr = S(1); Sg = S(2); 
Sb = S(3); 
M_rgb2xyz = [Sr * Xr, Sg * Xg, Sb * Xb; Sr * Yr, Sg * Yg, Sb * Yb; Sr * Zr, Sg * Zg, Sb * Zb]; 
M_xyz2rgb = inv(M_rgb2xyz); 
rgbImg = colorSpaceTransform(xyzImg, M_xyz2rgb); 
figure(2), clf; imshow(rgbImg, 'XData', linspace(0,1,resolution), ... 
    'YData', linspace(0,1,resolution)); axis on; set(gca, 'YDir', 'Normal'); 
xlabel('x'); 
ylabel('y'); 
